...
app.get('/api/get/:id', (req, res) => {
  new Promise((resolve, reject) => {
    fs.readFile(catalog_path, 'utf-8', (err, data) => {
      if (err || JSON.parse(data).length <= 0 || Number(req.params.id) === 0) {
        reject(err || 'Ошибка загрузки товара!')
      } else {
        JSON.parse(data).length
          ? resolve(JSON.parse(data).find(p => +p.id === +req.params.id))
          : reject('Нет таких товаров!');
      }
    })
  })
  ...
  fs.readFile(catalog_path, 'utf-8', (err, data) => {
    if (err || JSON.parse(data).length <= 0) {
      ...
      let
        ...
        result = JSON.parse(data).filter(item => item.title.toUpperCase().indexOf(query.toUpperCase()) + 1);
...
app.get('/api/cart', (req, res) => {
  new Promise((resolve, reject) => {
    fs.readFile(cart_path, 'utf-8', (err, data) => {
      err ? reject('Ошибка загрузки корзины! ' + err ) : resolve(data);
    });
  })
