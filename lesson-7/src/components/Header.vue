<template>
  <header class="header">
    <div class="container header__wrp">
      <div class="header__wrp-controls">
        <router-link class="header__logo-link" to="/">
          <svg class="header__logo-icon" width="44" height="38" viewBox="0 0 44 38" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
            <rect width="44" height="38" fill="url(#pattern0)"/>
            <defs>
              <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                <use xlink:href="#image0" transform="scale(0.0227273 0.0263158)"/>
              </pattern>
              <image id="image0" width="44" height="38" xlink:href="data:image/png;base64,"/>
            </defs>
          </svg>
        </router-link>
        <search @search="searchProduct"></search>
      </div>
      <div class="header__wrp-controls">
        <mainmenu></mainmenu>
        <router-link class="header__link" to="/login">
          <svg-icon class="header__icon" :size="[32,23]" :name="'login'"></svg-icon>
        </router-link>
        <button class="header__link">
          <span class="header__quantity" id="quantity-icon">{{ count }}</span>
          <svg-icon class="header__icon" :size="[33,29]" :name="'cart'"></svg-icon>
        </button>
        <cart :totalprice="price" :cart="cart" @change-count="changeCount"></cart>
      </div>
    </div>
  </header>
</template>

<script>
import SvgIcon from './Svg'
import search from './Search'
import mainmenu from './MainMenu'
import cart from './Cart'

export default {
  name: 'Header',
  props: ['cart', 'price', 'count'],
  components: { search, mainmenu, cart, SvgIcon },
  methods: {
    changeCount (id, type, count) {
      this.$emit('change-count', id, type, count)
    }
  }
}
</script>

<style lang="sass">
.header
  background-color: $black-back
  position: relative
  flex: 0 0 auto

  &__wrp
    @include flex-space()
    min-height: 75px

  &__wrp-controls
    @include flex-center()

  &__logo-link
    margin-right: 41px

  &__search-form
    display: inline-block

  &__link
    margin-left: 31px
    position: relative
    background-color: transparent
    border: none
    @include mobile
      display: none

  &__icon
    fill: $white-icon

  &__menu-btn:focus + &__nav,
  &__nav:hover
    display: block

  &__link:focus + &__card-menu,
  &__card-menu:hover
    display: block

  &__quantity
    background-color: $pink-text
    text-decoration: none
    @include size(auto, auto)
    padding: 1px 6px
    @include offset($top:-10,$left:26)
    border-radius: 10px
    color: white
    @include font(16,19,700)
</style>
